...
const Respond = () => {
  ...
  function mapRespondError(
    ...
    if (data.error && typeof data.error === "string") {
      ...
      if (err.includes("missing email")) {
        ...
        err.includes("missing cafe_id") ||
        err.includes("missing cafe id")
      ) {
        msg =
          _i18n.language === "nl"
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        if (!token) {
          if (!didCancel) {
            setError(
              getRespondErrorMessage(t, "respond.invalidInvitation", null),
            );
            setLoading(false);
          }
          return;
        ...
        if (!invitation) {
          if (!didCancel) {
            setError(
              getRespondErrorMessage(t, "respond.expiredOrMissing", null),
            );
            setLoading(false);
          }
          return;
        ...
        if (
          ...
          Array.isArray(invitation.date_time_options)
        ) {
          invitation.date_time_options.forEach(
            (opt: { date: string; times: string[] }) => {
    ...
    const timeout = setTimeout(() => {
      if (!didCancel && loading) {
        setError(
          "Loading is taking too long. Please check your invite link or try again later.",
        );
        setLoading(false);
      }
    }, 900000); // 15 minutes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.email)
      missing.push(_i18n.language === "nl" ? "e-mailadres" : "email address");
    if (!formData.selectedTime)
      missing.push(_i18n.language === "nl" ? "tijd" : "time");
  ...
  if (error) {
    return (
      <div className="max-w-2xl mx-auto text-center py-12 text-red-500">
        {error}
      </div>
    );
  }
