  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (profile) {
        ...
        setDisplayName(profile.fullname || generateRandomName());
      ...
      try {
        ...
        if (meetupsError) {
          ...
          setMeetupsError(
            t('account.errorLoadingMeetupsDetails', { details: meetupsError.message }),
          );
          setMyMeetups([]);
        } else {
        ...
        setMeetupsError(
          t('account.errorLoadingMeetupsDetails', {
    ...
    <div className="max-w-2xl mx-auto py-8 px-2 flex flex-col gap-8">
      ...
      <div className="bg-white/90 rounded-2xl shadow p-6 flex flex-col gap-6 border border-[#b2dfdb]/30">
        <h2 className="text-xl font-bold text-primary-700 mb-2 flex items-center gap-2">
          📝 {t('account.profileInfo').toLowerCase() || 'Profielinformatie'}
        </h2>
        {/* Name */}
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="font-medium text-gray-700">
              {t('account.nameLabel').toLowerCase()}:
            </span>
            {!editName ? (
              <>
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="font-medium text-gray-700">
              {t('account.emailLabel').toLowerCase()}:
            </span>
            {!editEmail ? (
              <>
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="font-medium text-gray-700">
              {t('account.genderLabel').toLowerCase()}:
            </span>
            {!editGender ? (
              <>
                <span className="text-primary-700 font-semibold">
                  {genderOptions.find((opt) => opt.value === gender)?.label ||
                    gender ||
                    t('account.noGender').toLowerCase()}
                </span>
                <button
              ...
              <>
                <div className="flex gap-3 mt-1">
                  {genderOptions.map((opt) => (
                    <label
                      key={opt.value}
                      className={`cursor-pointer px-4 py-2 rounded-xl border-2 font-medium ${pendingGender === opt.value ? 'bg-[#b2dfdb]/60 border-[#ff914d] text-primary-700' : 'bg-white border-gray-300 text-gray-700'} transition`}
                    >
                      <input
                        type="radio"
                        name="gender"
          ...
          <div className="flex items-center gap-2">
            ...
            {!editAge ? (
              ...
              <>
                ...
                />
                <div className="text-xs text-gray-500 mt-1">
                  {t('account.ageEditHint').toLowerCase() ||
                    'Leeftijd aanpassen? Scroll of typ hier.'}
                </div>
                <div className="flex gap-2 mt-1">
                  <button
                    className="btn-primary px-4 py-1"
      ...
      <div className="bg-white/90 rounded-2xl shadow p-6 flex flex-col gap-4 border border-[#b2dfdb]/30">
        <h2 className="text-xl font-bold text-primary-700 mb-2 flex items-center gap-2">
          🔑 {t('account.changePassword').toLowerCase()}
        </h2>
        {!showPwForm ? (
          <button className="btn-secondary px-4 py-2 w-fit" onClick={() => setShowPwForm(true)}>
            {t('account.changePassword').toLowerCase()}
          </button>
        ) : (
          <form className="flex flex-col gap-2" onSubmit={handlePasswordSave} autoComplete="off">
            ...
            />
            <div className="flex gap-2 mt-1">
              <button className="btn-primary px-4 py-1" type="submit" disabled={pwSaving}>
                {t('account.saveChanges').toLowerCase()}
              </button>
              <button
      ...
      <div className="bg-white/90 rounded-2xl shadow p-6 flex flex-col gap-4 border border-[#b2dfdb]/30">
        <h2 className="text-xl font-bold text-primary-700 mb-2 flex items-center gap-2">
          ⚙️ {t('account.preferencesTitle').toLowerCase()}
        </h2>
        <div className="flex flex-col gap-4 items-start max-w-xs mx-auto">
          ...
          >
            <span>💾</span>{' '}
            {prefsSaving ? t('common.loading').toLowerCase() : t('account.save').toLowerCase()}
          </button>
          {prefsMsg && (
      ...
      <div className="bg-white/90 rounded-2xl shadow p-6 flex flex-col gap-4 border border-[#b2dfdb]/30">
        ...
        {myMeetups.length === 0 && !meetupsLoading && !meetupsError && (
          <div className="text-gray-600 text-center">
            {t('dashboard.noMeetups').toLowerCase() || 'Nog geen meetups gepland. Waarom niet?'}
          </div>
        )}
        <ul className="space-y-3">
          {myMeetups.map((m) => (
            <li
              key={m.id}
              className="bg-[#fff7f3] rounded-xl shadow p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between border border-[#ff914d]/10"
            >
              <div>
                <span className="font-semibold text-primary-700">{m.selected_date}</span>
                {m.selected_time && <span> &bull; {m.selected_time}</span>}
                {m.cafe_id && <span> &bull; Café {m.cafe_id}</span>}
              </div>
              <span
      ...
      <div className="bg-red-50 border border-red-200 rounded-2xl shadow p-6 flex flex-col gap-4 items-center">
        <h2 className="text-xl font-bold text-red-700 mb-2 flex items-center gap-2">
          ⚠️ {t('account.deleteAccount').toLowerCase()}
        </h2>
        <button
        ...
        >
          <span>🗑️</span>{' '}
          {deleting ? t('common.loading').toLowerCase() : t('account.deleteAccount').toLowerCase()}
        </button>
        {showDeleteConfirm && (
          <div className="bg-white border border-red-300 rounded-xl p-6 mt-4 max-w-md w-full text-center shadow-xl">
            <div className="text-red-700 font-semibold mb-4">
              {t('account.deleteAccountConfirm').toLowerCase()}
            </div>
            <div className="flex gap-4 justify-center">
              ...
              >
                ...
                {deleting
                  ? t('common.loading').toLowerCase()
                  : t('account.deleteAccount').toLowerCase()}
              </button>
            </div>
