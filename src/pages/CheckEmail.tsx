  ...
  return (
    <main className="max-w-md mx-auto px-2 sm:px-0 py-8">
      <div className="card bg-white rounded-xl shadow-md p-6 flex flex-col items-center text-center">
        ...
        <p className="text-gray-700 mb-4 text-base sm:text-lg">{t('checkEmail.description')}</p>
        <button className="btn-secondary w-full mb-2" onClick={openEmailApp}>
        ...
        >
          {status === 'loading' ? t('checkEmail.checking') : t('checkEmail.cta')}
        </button>
        {status !== 'idle' && (
          <FormStatus
        ...
        <div className="mt-6 text-xs text-gray-500">{t('checkEmail.notReceived')}</div>
      </div>
    </main>
  );
};
